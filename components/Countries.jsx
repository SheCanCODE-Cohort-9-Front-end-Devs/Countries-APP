...
function Countries() {
    ...
    const handleSearchChange = (e) => {
        ...
        filterCountries(value, continentQuery);
    };

    const handleContinentChange = (e) => {
        const { value } = e.target;
        setContinentQuery(value);

        filterCountries(searchQuery, value);
    };

    const filterCountries = (nameQuery, continentQuery) => {
        const filtered = countryData.filter(country =>
            country.name.common.toLowerCase().includes(nameQuery.toLowerCase()) &&
            (continentQuery === '' || country.region.toLowerCase().includes(continentQuery.toLowerCase()))
        );
        ...
        <div className="w-[90%]">
            <div className="flex justify-between">
                ...
                <input
                    ...
                    className="color-black mt-5 mr-10"
                    value={searchQuery}
            ...
            <p className="text-xl px-10 pb-10">Page {currentPage} of {Math.ceil(filteredCountries.length / countriesPerPage)}</p>
                ...
                <div className="flex justify-center my-5 pt-6">
                    <ul className="flex">
                        ...
                        {Array.from({ length: Math.ceil(filteredCountries.length / countriesPerPage) }, (_, index) => (
                            <li key={index + 1} className={`mx-1 cursor-pointer border-2 p-2 ${currentPage === index + 1 ? 'bg-blue-500 text-white' : ''}`} onClick={() => paginate(index + 1)}>
                                {index + 1}
                            </li>
                        ))}
